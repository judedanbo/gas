...
export default defineEventHandler(async (event) => {
  ...
  const conditions = [eq(schema.managementTeam.slug, slug)]
  if (excludeId) {
    conditions.push(ne(schema.managementTeam.id, excludeId))
  }
  ...
  while (counter <= 100) {
    ...
    const suggestionConditions = [eq(schema.managementTeam.slug, suggestion)]
    if (excludeId) {
      suggestionConditions.push(ne(schema.managementTeam.id, excludeId))
    }
