...
export default defineEventHandler(async (event) => {
  ...
  const conditions = [eq(schema.publications.slug, slug)]
  if (excludeId) {
    conditions.push(ne(schema.publications.id, excludeId))
  }
  ...
  while (counter <= 100) {
    ...
    const suggestionConditions = [eq(schema.publications.slug, suggestion)]
    if (excludeId) {
      suggestionConditions.push(ne(schema.publications.id, excludeId))
    }
