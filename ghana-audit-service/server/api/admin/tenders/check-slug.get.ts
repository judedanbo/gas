...
export default defineEventHandler(async (event) => {
  ...
  const conditions = [eq(schema.tenders.slug, slug)]
  if (excludeId) {
    conditions.push(ne(schema.tenders.id, excludeId))
  }
  ...
  while (counter <= 100) {
    ...
    const suggestionConditions = [eq(schema.tenders.slug, suggestion)]
    if (excludeId) {
      suggestionConditions.push(ne(schema.tenders.id, excludeId))
    }
