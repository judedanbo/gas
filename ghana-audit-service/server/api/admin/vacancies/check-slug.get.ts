...
export default defineEventHandler(async (event) => {
  ...
  const conditions = [eq(schema.vacancies.slug, slug)]
  if (excludeId) {
    conditions.push(ne(schema.vacancies.id, excludeId))
  }
  ...
  while (counter <= 100) {
    ...
    const suggestionConditions = [eq(schema.vacancies.slug, suggestion)]
    if (excludeId) {
      suggestionConditions.push(ne(schema.vacancies.id, excludeId))
    }
