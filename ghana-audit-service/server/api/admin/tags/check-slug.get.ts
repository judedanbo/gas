...
export default defineEventHandler(async (event) => {
  ...
  const conditions = [eq(schema.tags.slug, slug)]
  if (excludeId) {
  ...
  while (counter <= 100) {
    ...
    const suggestionConditions = [eq(schema.tags.slug, suggestion)]
    if (excludeId) {
      suggestionConditions.push(ne(schema.tags.id, excludeId))
    }
