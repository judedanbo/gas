...
export default defineEventHandler(async (event) => {
  ...
  const conditions = [eq(schema.auditReports.slug, slug)]
  if (excludeId) {
    conditions.push(ne(schema.auditReports.id, excludeId))
  }
  ...
  while (counter <= 100) {
    ...
    const suggestionConditions = [eq(schema.auditReports.slug, suggestion)]
    if (excludeId) {
      suggestionConditions.push(ne(schema.auditReports.id, excludeId))
    }
