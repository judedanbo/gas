...
export default defineEventHandler(async (event) => {
  ...
  const conditions = [eq(schema.newsArticles.slug, slug)]
  if (excludeId) {
    conditions.push(ne(schema.newsArticles.id, excludeId))
  }
  ...
  while (counter <= 100) {
    ...
    const suggestionConditions = [eq(schema.newsArticles.slug, suggestion)]
    if (excludeId) {
      suggestionConditions.push(ne(schema.newsArticles.id, excludeId))
    }
