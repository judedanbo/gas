    ...
    <ul class="flex items-center gap-1 list-none m-0 p-0">
      ...
      >
        <NuxtLink
          ...
          :class="{ 'text-primary dark:text-primary-light bg-gray-50 dark:bg-gray-700': isActive(item.href) }"
        >
          {{ item.label }}
        </NuxtLink>
        ...
        <Transition name="dropdown">
          <div
            ...
            @mouseleave="closeDropdown"
          >
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl min-w-[280px] overflow-hidden">
              <ul class="list-none m-0 p-2">
                <li v-for="child in item.children" :key="child.label">
                  <NuxtLink
                    ...
                    @click="closeDropdown"
                  >
                    <Icon v-if="child.icon" :name="child.icon" class="w-5 h-5 flex-shrink-0 mt-0.5 text-primary dark:text-primary-light" aria-hidden="true" />
                    <span class="flex flex-col gap-1">
                      <span class="font-medium text-gray-900 dark:text-white">{{ child.label }}</span>
                      <span v-if="child.description" class="text-sm text-gray-500 dark:text-gray-400">
                        {{ child.description }}
                      </span>
                    </span>
                  </NuxtLink>
...
const navigationItems: NavItem[] = [
  {
    ...
    href: '/about',
    children: [
      { label: 'The Service', href: '/about/the-service', icon: 'heroicons:building-library', description: 'Learn about our history and mandate' },
      { label: 'Past Auditors-General', href: '/about/past-auditors-general', icon: 'heroicons:user', description: 'Our leadership through the years' },
      { label: 'Management Team', href: '/about/management-team', icon: 'heroicons:user-group', description: 'Current leadership and directors' },
      { label: 'Departmental Profile', href: '/about/departmental-profile', icon: 'heroicons:building-office', description: 'Our organizational structure' }
    ]
  },
  {
    label: "A-G's Reports",
  ...
  },
  {
    ...
    href: '/publications',
    children: [
      { label: 'Press Statements', href: '/publications/press-statements', icon: 'heroicons:newspaper', description: 'Official announcements' },
      { label: 'Bulletins', href: '/publications/bulletins', icon: 'heroicons:document-text', description: 'Quarterly publications' },
      { label: 'Auditing Guidelines', href: '/publications/guidelines', icon: 'heroicons:clipboard-document-list', description: 'Standards and procedures' },
      { label: 'AMIS Manuals', href: '/publications/amis-manuals', icon: 'heroicons:document-text', description: 'Audit management manuals' },
      { label: 'PFM Strategy', href: '/publications/pfm-strategy', icon: 'heroicons:flag', description: '2022-2026 strategic plan' },
      { label: 'Applicable Laws', href: '/publications/applicable-laws', icon: 'heroicons:scale', description: 'Legal framework' }
    ]
  },
  {
    label: 'Media Centre',
    href: '/media',
    children: [
      { label: 'News', href: '/media/news', icon: 'heroicons:newspaper', description: 'Latest updates and stories' },
      { label: 'Events', href: '/media/events', icon: 'heroicons:calendar', description: 'Upcoming and past events' },
      { label: 'Photo Gallery', href: '/media/gallery', icon: 'heroicons:camera', description: 'Visual documentation' },
      { label: 'Videos', href: '/media/videos', icon: 'heroicons:video-camera', description: 'Video content and interviews' }
    ]
  },
  {
    label: 'Careers',
