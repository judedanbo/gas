<template>
  <div>
    ...
    <div v-else-if="!currentItem" class="text-center py-12">
      ...
      <NuxtLink to="/admin/events" class="btn btn-primary">Back to Events</NuxtLink>
    </div>
    <template v-else>
...
<script setup lang="ts">
  ...
  const { currentItem, loading, saving, error, fieldErrors, fetchOne, update } =
    useAdminCrud<AdminEvent>('events')
  ...
  // Check slug availability with debounce (excludes current event)
  async function checkSlugAvailability(slug: string) {
    ...
    try {
      ...
      const response = await get<{ available: boolean; suggestion?: string }>('events/check-slug', {
        slug,
        excludeId: id
      })
  ...
  onMounted(async () => {
    ...
    if (item) {
      ...
      form.isVirtual = item.isVirtual
      form.registrationUrl = item.registrationUrl || ''
      form.thumbnail = item.thumbnail || ''
      form.isPublished = item.isPublished
      form.translations = item.translations || { en: { title: '', description: '', location: '' } }
    }
  })
