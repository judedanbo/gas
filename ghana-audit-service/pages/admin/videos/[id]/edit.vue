<template>
  <div>
    ...
    <div v-else-if="!currentItem" class="text-center py-12">
      <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">Video not found</h2>
      <NuxtLink to="/admin/videos" class="btn btn-primary">Back to Videos</NuxtLink>
    </div>
    <template v-else>
...
<script setup lang="ts">
  ...
  const { currentItem, loading, saving, error, fieldErrors, fetchOne, update } =
    useAdminCrud<AdminVideo>('videos')
