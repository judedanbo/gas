<template>
  <div>
    ...
    <div v-else-if="!currentItem" class="text-center py-12">
      ...
      <NuxtLink to="/admin/management-team" class="btn btn-primary"
        >Back to Management Team</NuxtLink
      >
    </div>
    <template v-else>
...
<script setup lang="ts">
  ...
  // Initialized flag to prevent watchers from resetting values during initial load
  ...
  // Check slug availability with debounce (excludes current member)
  ...
  // Reset child selection when parent changes (only after init)
  ...
  onMounted(async () => {
    ...
    if (item) {
      ...
      // Pre-populate cascading parent for district auditors
      ...
      // Pre-populate cascading parent for branch heads
