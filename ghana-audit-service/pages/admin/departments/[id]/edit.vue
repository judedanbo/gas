<template>
  <div>
    ...
    <div v-else-if="!currentItem" class="text-center py-12">
      <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">Department not found</h2>
      <NuxtLink to="/admin/departments" class="btn btn-primary">Back to Departments</NuxtLink>
    </div>
    <template v-else>
...
<script setup lang="ts">
  ...
  // Check slug availability with debounce (excludes current department)
  async function checkSlugAvailability(slug: string) {
    ...
    try {
      const response = await $fetch<{ available: boolean; suggestion?: string }>(
        '/api/admin/departments/check-slug',
  ...
  onMounted(async () => {
    ...
    if (item) {
      form.slug = item.slug
      form.headId = item.headId || undefined
      form.displayOrder = item.displayOrder
      form.translations = item.translations || { en: { name: '', description: '' } }
      form.functions =
        item.functions?.map((f) => ({
          displayOrder: f.displayOrder,
          translations: {
            en: { description: f.translations?.en?.description || '' },
            ak: { description: f.translations?.ak?.description || '' }
          }
        })) || []
    }
  })
