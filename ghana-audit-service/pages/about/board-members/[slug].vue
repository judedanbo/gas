    ...
    <CommonBreadcrumb
      :crumbs="[
        ...
        { label: 'Board Members', path: '/about/board-members' },
        {
          ...
          path: `/about/board-members/${route.params.slug}`
        }
      ]"
    />
    ...
    <div v-else-if="error" class="section">
      <div class="container text-center py-12">
        ...
        <p class="text-gray-600 dark:text-gray-400 mb-6">
          The board member you're looking for doesn't exist or has been removed.
        </p>
        <NuxtLink to="/about/board-members" class="btn-primary"> View Board Members </NuxtLink>
      </div>
    </div>
      ...
      <section class="bg-gradient-to-br from-primary to-primary-dark text-white py-12">
        <div class="container">
          ...
          >
            ...
            <div>
              <UiBadge
                :variant="member.role === 'chairperson' ? 'accent' : 'primary'"
                size="lg"
                class="mb-3"
              >
                {{ member.role === 'chairperson' ? 'Chairperson' : 'Board Member' }}
              </UiBadge>
              <h1 class="text-3xl md:text-4xl font-heading font-bold text-white mb-2">
                {{ member.name }}
              </h1>
              <p v-if="member.title" class="text-xl text-white/90 mb-4">{{ member.title }}</p>
        ...
        <div class="container">
          <div class="max-w-6xl mx-auto flex flex-col lg:flex-row gap-10">
            ...
            <div class="flex-1 min-w-0">
              <div v-if="bioSections.length > 0" class="space-y-8">
                <div
                      ...
                      >
                        ...
                        <div class="min-w-0">
                          ...
                          <p class="text-xs text-gray-500 dark:text-gray-400 truncate m-0">
                            {{ m.role === 'chairperson' ? 'Chairperson' : 'Member' }}
                          </p>
                        </div>
                      </NuxtLink>
                    </li>
...
<script setup lang="ts">
  ...
  const {
    ...
    pending,
    error
  } = await useFetch<BoardMember>(`/api/board-members/${route.params.slug}`)

  const { data: allMembers } = await useFetch<BoardMember[]>('/api/board-members')
  ...
  useSeoMeta({
    ...
    description: () =>
      member.value
        ...
        : 'Board member profile at the Ghana Audit Service.'
  })
</script>
